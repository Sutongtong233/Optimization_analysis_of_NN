...
def linear_inter_ini_opt(W_0_ini, W_0_opt, model, alpha):
    model_inter = copy.deepcopy(model)
    for name, parameters in model_inter.named_parameters():
        if name == 'gcn1.weight': 
            parameters.data = alpha*W_0_ini + (1-alpha)*W_0_opt
    # model_inter.gcn1.weight = torch.nn.Parameter(alpha*W_0_ini + (1-alpha)*W_0_opt)
...
if __name__=='__main__':
    ...
    W_0_ini, W_1_ini, W_ini = get_model_params(model)
    acc_before, output_before, predict_before = train(model)
    W_0_opt, W_1_opt, W_opt = get_model_params(model)
    print(acc_before)


    # Exp 1: optimization path is easy
    loss_ls = []
    n_inter = 50
    for alpha in np.linspace(0, 2, n_inter):
        loss_ls.append(linear_inter_ini_opt(W_0_ini, W_0_opt, model, alpha))
    # print(loss_ls)
    plt.plot(np.linspace(0, 2, n_inter), loss_ls)
    plt.savefig("./GNN-Perturbation/result/linear_inter_ini_opt.png")
